...
function EmailSection() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      email: form.email.value, // Access the email field
      subject: form.subject.value, // Access the subject field
      message: form.message.value, // Access the message field
    };
  ...
  return (
    <section
      id="contact"
      className="grid md:grid-cols-2 my-12 md:my-12 gap-4 py-24 relative"
    >
      ...
      <div>
        <form className="flex flex-col" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="bg-brown-300 hover:bg-brown-400 text-white font-medium py-2.5 px-5 rounded-lg w-full "
          >
            {" "}
            Send message
          </button>
          {emailSubmitted && (
            <p className="text-green-500 text-sm mt-2 justify-center ">
